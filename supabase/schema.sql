...
create policy "sets owner all" on public.study_sets for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "sources owner all" on public.sources;
create policy "sources owner all" on public.sources for all using (
...
create policy "materials owner all" on public.study_materials for all using (
  exists (select 1 from public.study_sets s where s.id = study_set_id and s.user_id = auth.uid())
) with check (
  exists (select 1 from public.study_sets s where s.id = study_set_id and s.user_id = auth.uid())
);
...
create policy "conv owner all" on public.conversations for all using (
  exists (select 1 from public.study_sets s where s.id = study_set_id and s.user_id = auth.uid())
) with check (
  exists (select 1 from public.study_sets s where s.id = study_set_id and s.user_id = auth.uid())
);
